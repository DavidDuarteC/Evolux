...
export default function Sidebar({ mobileOpen, onMobileClose }) {
    ...
    const sidebarContent = (
        <div
            className="h-full w-full glass-card flex flex-col relative rounded-2xl md:rounded-2xl transition-all duration-300 px-2 py-3"
        ...
        >
            ...
            <button
                ...
                className={`flex items-center mb-2 shrink-0 h-[52px] text-left transition-colors hover:bg-white/5 relative group rounded-xl mx-auto
                    ${isCollapsed ? 'justify-center w-10 p-0' : 'w-full px-1.5'}
                `}
            >
                <div
                    className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-base shadow-lg shrink-0 transition-all duration-300 z-20 relative overflow-hidden"
                ...
                <AnimatePresence mode="wait">
                    {!isCollapsed && (
                        <motion.div
                            ...
                            className="overflow-hidden ml-2 min-w-0"
                        >
                            <h3 className="font-medium text-xs truncate transition-colors" style={{ color: 'var(--text-primary)' }}>{user.name}</h3>
                            <p className="text-[10px] truncate" style={{ color: 'var(--text-muted)' }}>{t('sidebar.verPerfil')}</p>
                        </motion.div>
                    )}
                </AnimatePresence>
            </button>
                            ...
                            <AnimatePresence mode="wait">
                                {!isCollapsed && (
                                    <motion.span
                                        ...
                                        exit={{ opacity: 0, x: -10 }}
                                        transition={{ duration: 0.2 }}
                                        className={`text-xs font-medium whitespace-nowrap z-10 ml-2 truncate ${isActive ? '' : 'text-text-muted group-hover:text-white'}`}
